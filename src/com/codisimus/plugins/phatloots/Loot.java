...
public class Loot implements Comparable {
	 ...
	 * @param amountLower
	 *            The lower bound of the stack size of the item
	 * @param amountUpper
	 *            The upper bound of the stack size of the item
	 * @param probability
	 *            The chance of looting the item
	 */
	 ...
	 * 
	 * @param item
	 *            The ItemStack that will be looted
	 * @param bonus
	 *            The amount of extra Items that may be looted
	 * @param probability
	 *            The chance of looting the item
	 */
	 ...
	 * 
	 * @param enchantments
	 *            The Enchantments to be added and their levels
	 */
	 ...
	 * 
	 * @param name
	 *            The Name of the Item Description File
	...
	public boolean setName(String name) {
		...
		if (item.getType() == Material.WRITTEN_BOOK) {
			...
			try {
				fos = new FileOutputStream(PhatLoots.dataFolder + "/Books/"
						+ title + ".properties");
				book.store(fos, null);
				fos.close();
			...
			if (name.isEmpty()) {
				...
				String folder = item.getType()
						+ item.getEnchantments().toString();
			...
			File file = new File(PhatLoots.dataFolder + "/Item Descriptions/"
					+ name + ".txt");
			if (file.exists()) {
				return true;
			}
	...
	public ItemStack getItem() {
		...
		if (autoEnchant) {
			...
			for (Enchantment enchantment : enchantments) {
				if (enchantmentConfig.contains(type + '.' + enchantment)) {
					ConfigurationSection config = enchantmentConfig
							.getConfigurationSection(type + '.' + enchantment);
		...
		if (!name.isEmpty()) {
			ItemMeta meta = clone.hasItemMeta() ? clone.getItemMeta()
					: PhatLoots.server.getItemFactory().getItemMeta(
							clone.getType());
			...
			if (clone.getType() == Material.WRITTEN_BOOK) {
				...
				try {
					fis = new FileInputStream(PhatLoots.dataFolder + "/Books/"
							+ name + ".properties");
					book.load(fis);
				...
				if (name.equals("Random")) {
					String folder = clone.getType()
							+ clone.getEnchantments().toString();
					File dir = new File(PhatLoots.dataFolder
							+ "/Item Descriptions/" + folder);
					...
					// tag.setString(ITEM_DESCRIPTION, fileName.substring(0,
					// fileName.length() - 4));
	...
	private String getTieredName(ItemStack item) {
		...
		case LEATHER_BOOTS:
			...
			lore = loreConfig.getString(type + '.' + enchantment.getName()
					+ '.' + level);
			if (lore != null) {
				nameBuiler.insert(0, ' ');
				nameBuiler.insert(0, lore);
			}
			enchantment = enchantments.containsKey(Enchantment.THORNS) ? Enchantment.THORNS
					: Enchantment.DURABILITY;
			level = getLevel(enchantments, enchantment);
			lore = loreConfig.getString(type + '.' + enchantment.getName()
					+ '.' + level);
			...
			enchantment = getTrump(enchantments,
		...
		case WOOD_SWORD:
			type = SWORD;
			enchantment = getTrump(enchantments, Enchantment.DAMAGE_ARTHROPODS,
					Enchantment.DAMAGE_ALL);
			level = getLevel(enchantments, enchantment);
			lore = loreConfig.getString(type + '.' + enchantment.getName()
					+ '.' + level);
			if (lore != null) {
				nameBuiler.replace(nameBuiler.length() - 5,
						nameBuiler.length(), lore);
			}
			enchantment = enchantments.containsKey(Enchantment.FIRE_ASPECT) ? Enchantment.FIRE_ASPECT
					: Enchantment.DURABILITY;
			level = getLevel(enchantments, enchantment);
			lore = loreConfig.getString(type + '.' + enchantment.getName()
					+ '.' + level);
			...
			enchantment = getTrump(enchantments, Enchantment.KNOCKBACK,
		...
		if (tier > tierNotify) {
			PhatLoots.logger.info(name + " [Tier " + tier
					+ "] has been generated");
		}
		return name;
	}
	 ...
	 * indicates an optional additional field Amount[-Amount] of Name [with data
	 * Durability] [with enchantments
	 * Enchantment1(level)&Enchantment2(level)...] @ Probability%
	 * 
	...
	public String toInfoString() {
		...
		string += " of " + (name.isEmpty() ? item.getType().name() : name);
		...
		string += " @ "
				+ (Math.floor(probability) == probability ? String
						.valueOf((int) probability) : String
						.valueOf(probability)) + "%";
	 ...
	 * [(DyedColor|SkullOwner)]MaterialID[+Name]
	 * 'Durability[+Enchantment1(level)&Enchantment2(level)...]'Amount[-Amount]'Probabilit
	 * y
	 * 
	...
	public String toString() {
		...
		if (id >= 298 && id <= 301) {
			if (item.hasItemMeta()) {
				string = "("
						+ ((LeatherArmorMeta) item.getItemMeta()).getColor()
								.asRGB() + ")" + string; // :DyedColor
			}
		} else if (id == 144) {
			if (item.hasItemMeta()) {
				string = "(" + ((SkullMeta) item.getItemMeta()).getOwner()
						+ ")" + string; // :SkullOwner
			}
		}
		...
		String prob = Math.floor(probability) == probability ? String
				.valueOf((int) probability) // Remove ".0"
	...
	public boolean equals(Object object) {
		...
		return loot.name.equals(name)
				...
				&& loot.item.getEnchantments().entrySet()
						.equals(item.getEnchantments().entrySet());
	}
	...
	public int hashCode() {
		...
		hash = 97
				* hash
				+ (int) (Double.doubleToLongBits(this.probability) ^ (Double
						.doubleToLongBits(this.probability) >>> 32));
	...
	private Enchantment getTrump(Map<Enchantment, Integer> enchantments,
	...
	private int getLevel(Map<Enchantment, Integer> enchantments,
			Enchantment enchantment) {
