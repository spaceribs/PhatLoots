...
public class PhatLootsConfig {
	...
	public static void load() {
		try {
			File file = new File(PhatLoots.dataFolder + File.separator
					+ "lores.yml");
			if (!file.exists()) {
		...
		try {
			...
			PhatLoots.useRestricted = loadBool(
					"PermissionNeededOnlyForRestrictedPhatLoots", false);
			PhatLoots.restricted.addAll(Arrays.asList(loadString(
					"RestrictedPhatLoots", "Rare, Epic, Donator").split(", ")));

			Loot.tierNotify = loadInt("MinimumTierNotification", 50);

			PhatLoot.onlyDropOnPlayerKill = loadBool(
					"OnlyDropLootWhenKilledByPlayer", false);
			PhatLoot.replaceMobLoot = loadBool("ReplaceMobLoot", true);
			PhatLoot.chanceOfDrop = (float) (loadDouble(
					"MobLootDropPercentage", 20.0) / 100);
			PhatLoots.autoLoot = loadBool("AutoLoot", false);
			PhatLoot.displayTimeRemaining = loadBool("DisplayTimeRemaining",
					true);
			PhatLoot.displayMobTimeRemaining = loadBool(
					"DisplayMobTimeRemaining", true);

			PhatLootsCommand.setUnlockable = loadBool("SetChestsAsUnlockable",
					true);

			String[] defaultResetTime = loadString("DefaultResetTime",
					"1'0'0'0").split("'");
			...
			PhatLootsMessages.permission = loadString("PermissionMessage",
					string);
			PhatLootsMessages.experienceLooted = loadString(
					"ExperienceLootedMessage", string);
			PhatLootsMessages.moneyLooted = loadString("MoneyLootedMessage",
					string);
			PhatLootsMessages.autoLoot = loadString("AutoLootMessage", string);
			PhatLootsMessages.overflow = loadString("OverflowMessage", string);
			PhatLootsMessages.timeRemaining = loadString(
					"TimeRemainingMessage", string);
			PhatLootsMessages.mobTimeRemaining = loadString(
					"MobTimeRemainingMessage", string);
			PhatLootsMessages.mobDroppedMoney = loadString(
					"MobDroppedMoneyMessage", string);
			PhatLootsMessages.mobDroppedItem = loadString(
					"MobDroppedItemMessage", string);
	...
	private static String loadString(String key, String defaultString) {
		if (p.containsKey(key)) {
			...
			PhatLoots.logger
					.severe("Please regenerate the config.properties file (delete the old file to allow a new one to be created)");
			PhatLoots.logger
					.severe("DO NOT POST A TICKET FOR THIS MESSAGE, IT WILL JUST BE IGNORED");
	...
	private static int loadInt(String key, int defaultValue) {
		...
		try {
			...
			PhatLoots.logger.severe("The setting for " + key
