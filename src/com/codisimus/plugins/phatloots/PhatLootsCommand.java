...
public class PhatLootsCommand implements CommandExecutor {
	private static enum Action {
		HELP, MAKE, DELETE, LINK, UNLINK, TIME, GLOBAL, ROUND, ADD, REMOVE, MONEY, EXP, LIST, INFO, GIVE, RESET, RL
	}

	private static enum Help {
		CREATE, SETUP, LOOT
	}

	private static final HashSet<Byte> TRANSPARENT = Sets.newHashSet((byte) 0,
			(byte) 6, (byte) 8, (byte) 9, (byte) 10, (byte) 11, (byte) 26,
			(byte) 27, (byte) 28, (byte) 30, (byte) 31, (byte) 32, (byte) 37,
			(byte) 38, (byte) 39, (byte) 40, (byte) 44, (byte) 50, (byte) 51,
			(byte) 53, (byte) 55, (byte) 59, (byte) 64, (byte) 63, (byte) 65,
			(byte) 66, (byte) 67, (byte) 68, (byte) 69, (byte) 70, (byte) 71,
			(byte) 72, (byte) 75, (byte) 76, (byte) 77, (byte) 78, (byte) 85,
			(byte) 90, (byte) 92, (byte) 96, (byte) 101, (byte) 102,
			(byte) 104, (byte) 105, (byte) 106, (byte) 107, (byte) 108,
			(byte) 109, (byte) 111, (byte) 113, (byte) 114, (byte) 115,
			(byte) 117, (byte) 126, (byte) 127, (byte) 131, (byte) 132,
			(byte) 139, (byte) 140, (byte) 141, (byte) 142, (byte) 144,
			(byte) 145);
	 ...
	 * 
	 * @param sender
	 *            The CommandSender who may not be a Player
	 * @param command
	 *            The command that was executed
	 * @param alias
	 *            The alias that the sender used
	 * @param args
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		try {
			...
			if (phatLoot == null) {
				sender.sendMessage("§4PhatLoot §6" + args[0]
						+ "§4 does not exist");
				return true;
			}
			...
			if (sender instanceof Player) {
				...
				Inventory inventory = PhatLoots.server.createInventory(
						(Player) sender, 54, phatLoot.name);

				// Open the Inventory
				((Player) sender).openInventory(inventory);

				phatLoot.rollForLoot((Player) sender, new PhatLootChest(
						((Player) sender).getLocation().getBlock()), inventory);
			}
			return true;
		}
		...
		case DELETE:
			...
			if (args.length == 2) {
				...
				if (delete == null) {
					sender.sendMessage("§4PhatLoot §6" + args[1]
							+ "§4 does not exist");
				} else {
					PhatLoots.removePhatLoot(delete);
					sender.sendMessage("§5PhatLoot §6" + delete.name
							+ "§5 was deleted!");
				}
			} else {
				sendCreateHelp(sender);
		...
		case TIME:
			...
			case 5: // Name is not provided
				try {
					time(sender, null, Integer.parseInt(args[1]),
			...
			case 6: // Name is provided
				try {
					time(sender, args[1], Integer.parseInt(args[2]),
							Integer.parseInt(args[3]),
							Integer.parseInt(args[4]),
		...
		case ADD:
			...
			String phatLoot = null; // Name of the PhatLoot (may be set to
									// 'hand')
			String description = ""; // Name of the Description File (defaulted
										// to null or unused)
			int id = 0; // The ID of the Loot collection (defaulted to 0 ==
						// IndividualLoots)
			int baseAmount = 1; // Stack size of the Loot item (defaulted to 1)
			int bonusAmount = 1; // Amount to possibly increase the Stack size
									// of the Loot item (defaulted to 1)
			double percent = 100; // The chance of receiving the Loot item
									// (defaulted to 100)
			boolean autoEnchant = false; // Whether or not the Loot Item should
											// be automatically enchanted at
											// time of Looting
			...
			for (int i = 2; i < args.length; i++) {
				...
				case 'p':
					phatLoot = s;
					if (phatLoot == null) {
						sender.sendMessage("§4PhatLoot §6" + s
								+ "§4 does not exist");
						return true;
					}
					break;
				...
				case '#':
					...
					if (baseAmount == -1 || bonusAmount == -1) {
						sender.sendMessage("§6" + s
								+ "§4 is not a valid number or range");
						return true;
					}
				...
				case 'd':
					...
					if (data == -1) {
						sender.sendMessage("§6" + s
								+ "§4 is not a valid data/durability value");
						return true;
					}
		...
		case GIVE:
			...
			Inventory inventory = PhatLoots.server.createInventory(player, 54,
					pLoot.name);
			player.openInventory(inventory);
			pLoot.rollForLoot(player, new PhatLootChest(player.getLocation()
					.getBlock()), inventory);
			return true;
	 ...
	 * 
	 * @param sender
	 *            The CommandSender creating the PhatLoot
	 * @param name
	 *            The name of the PhatLoot being created (must not already
	 *            exist)
	 */
	public static void make(CommandSender sender, String name) {
		...
		if (PhatLoots.hasPhatLoot(name)) {
			sender.sendMessage("§4A PhatLoot named §6" + name
					+ "§4 already exists.");
			return;
		}
	 ...
	 * 
	 * @param sender
	 *            The CommandSender linking the Block they are targeting
	 * @param name
	 *            The name of the PhatLoot the Block will be linked to
	 */
	public static void link(CommandSender sender, String name) {
		...
		case CHEST:
			...
			if (inventory instanceof DoubleChestInventory) {
				chest = (Chest) ((DoubleChestInventory) inventory)
						.getLeftSide().getHolder();
		...
		sender.sendMessage("§5Target " + blockName
				+ " has been linked to PhatLoot §6" + name);
		phatLoot.save();
	}

	/**
	 * Finds all Chests within the selected RegionOwn Region and links them to
	 * the specified PhatLoot
	 * 
	 * @param player
	 *            The Player who has a Region selected
	 * @param name
	 *            The name of the PhatLoot the Chests will be linked to
	 */
	public static void regionLink(Player player, String name) {
		...
		player.sendMessage("§6" + chests
				+ "§5 chests have been linked to PhatLoot §5" + name);
		phatLoot.save();
	}
	 ...
	 * 
	 * @param sender
	 *            The CommandSender unlinking the Block they are targeting
	 */
	public static void unlink(CommandSender sender, String name) {
		...
		for (PhatLoot phatLoot : getPhatLoots(sender, name)) {
			...
			sender.sendMessage("§5Target " + blockName
					+ " has been unlinked from PhatLoot §6" + phatLoot.name);
			phatLoot.save();
		}
	}
	 ...
	 * 
	 * @param sender
	 *            The CommandSender modifying the PhatLoot
	 * @param name
	 *            The name of the PhatLoot to be modified or null to indicate
	 *            all linked PhatLoots
	 * @param days
	 *            The amount of days
	 * @param hours
	 *            The amount of hours
	 ...
	 */
	public static void time(CommandSender sender, String name, int days,
		...
		for (PhatLoot phatLoot : getPhatLoots(sender, name)) {
			...
			sender.sendMessage("§5Reset time for PhatLoot §6" + phatLoot.name
					+ "§5 has been set to §6" + days + " days, " + hours
	 ...
	 *            all linked PhatLoots
	 * @param add
	 *            True the Loot will be added, false if it will be removed
	 * @param lootID
	 *            The id of the Loot, 0 for individual loots
	 * @param loot
	 *            The Loot that will be added/removed
	 */
	public static void setLoot(CommandSender sender, String name, boolean add,
			int lootID, Loot loot) {
		...
		for (PhatLoot phatLoot : getPhatLoots(sender, name)) {
			...
			if (lootTable.contains(loot)) {
				...
				if (!add) {
					...
					if (lootID == 0) { // Individual Loot
						...
						sender.sendMessage("§6" + lootDescription
								+ "§5 removed as Loot from §6coll" + lootID
								+ "§5, §6"
			...
			if (add) {
				...
				if (lootID == 0) { // Individual Loot
					...
					sender.sendMessage("§6" + lootDescription
							+ "§5 added as Loot to §6coll" + lootID
							+ "§5, §6" + phatLoot.getPercentRemaining(lootID)
							+ "%§5 remaining");
				}
	 ...
	 *            all linked PhatLoots
	 * @param amount
	 *            The String of the range of the amount
	 */
	public static void setMoney(CommandSender sender, String name, String amount) {
		...
		if (lower == -1 || upper == -1) {
			sender.sendMessage("§6" + amount
					+ " §4is not a valid number or range");
			return;
		}
		...
		for (PhatLoot phatLoot : getPhatLoots(sender, name)) {
			...
			sender.sendMessage("§5Money for PhatLoot §6"
					+ phatLoot.name
					+ "§5 set to "
					+ (lower == upper ? "§6" : "a range from §6" + lower
							+ "§5 to §6") + upper);
			phatLoot.save();
		}
	}
	 ...
	 * @param add
	 *            True if the command is to be added
	 * @param cmd
	 *            The command to be added/removed
	 */
	public static void setCommand(CommandSender sender, String name,
			double percent, boolean add, String cmd) {
		if (cmd.startsWith("/")) {
			cmd = cmd.substring(1);
		}
		if (percent < 100) {
	 ...
	 * 
	 * @param sender
	 *            The CommandSender requesting the list
	 */
	...
	/**
	 * Displays the info of the specified PhatLoot If a name is not provided, a
	 * list of PhatLoots linked to the target Block is displayed
	 * 
	 * @param sender
	 *            The CommandSender requesting the info
	 * @param name
	 *            The name of the PhatLoot
	 */
	public static void info(CommandSender sender, String name) {
		...
		case 1: // Display information for the one PhatLoot
			...
			sender.sendMessage("§2Reset Time:§b " + phatLoot.days + " days, "
					+ phatLoot.hours + " hours, " + phatLoot.minutes
					+ " minutes, and " + phatLoot.seconds + " seconds.");
	...
	/**
	 * Reset the use times of the specified PhatLoot/PhatLootChest If a name is
	 * not provided, the target PhatLootChest is reset
	 * 
	 * @param sender
	 *            The CommandSender reseting the PhatLootChests
	 * @param name
	 *            The name of the PhatLoot
	 */
	public static void reset(CommandSender sender, String name) {
		...
		if (name != null) {
			...
			if (!PhatLoots.hasPhatLoot(name)) {
				if (sender != null) {
					sender.sendMessage("§4PhatLoot §6" + name
							+ "§4 does not exsist.");
				}
				return;
			}
			...
			if (sender != null) {
				sender.sendMessage("§5All Chests in PhatLoot §6" + name
						+ "§5 have been reset.");
			}
		} else if (sender != null) {
	 ...
	 * 
	 * @param sender
	 *            The CommandSender needing help
	 */
	private static void sendHelp(CommandSender sender) {
		sender.sendMessage("§e     PhatLoots Help Page:");
		sender.sendMessage("§2/" + command
				+ " <Name>§b Loot a virtual Chest for the given PhatLoot");
		sender.sendMessage("§2/" + command + " list§b List all PhatLoots");
		sender.sendMessage("§2/" + command
				+ " info [Name]§b List info of PhatLoot");
		sender.sendMessage("§2/" + command
				+ " give <Name> <Player>§b Force Sender to open a PhatLoot");
		sender.sendMessage("§2/" + command
				+ " reset§b Reset looted times for target Block");
		sender.sendMessage("§2/" + command
				+ " reset <Name>§b Reset looted times for PhatLoot");
		sender.sendMessage("§2/" + command
				+ " reset all§b Reset looted times for all PhatLoots");
		sender.sendMessage("§2/" + command
				+ " help create§b Display PhatLoots Create Help Page");
		sender.sendMessage("§2/" + command
				+ " help setup§b Display PhatLoots Setup Help Page");
		sender.sendMessage("§2/" + command
				+ " help loot§b Display PhatLoots Manage Loot Help Page");
	...
	private static void sendCreateHelp(CommandSender sender) {
		...
		sender.sendMessage("§2/" + command
				+ " make <Name>§b Create PhatLoot with given name");
		sender.sendMessage("§2/" + command
				+ " delete <Name>§b Delete PhatLoot");
		sender.sendMessage("§2/" + command
				+ " link <Name>§b Link target Chest/Dispenser with PhatLoot");
		sender.sendMessage("§2/" + command
				+ " unlink [Name]§b Unlink target Block from PhatLoot");
	}
	...
	private static void sendSetupHelp(CommandSender sender) {
		...
		sender.sendMessage("§2/"
				+ command
				+ " time [Name] <Days> <Hrs> <Mins> <Secs>§b Set cooldown time for PhatLoot");
		sender.sendMessage("§2/"
				+ command
				+ " time [Name] never§b Set PhatLoot to only be lootable once per chest");
		sender.sendMessage("§2/" + command
				+ " global [Name] true§b Set PhatLoot to a global cooldown");
		sender.sendMessage("§2/"
				+ command
				+ " global [Name] false§b Set PhatLoot to an individual cooldown");
		sender.sendMessage("§2/"
				+ command
				+ " round [Name] <true|false>§b Set if cooldown times should round down (ex. Daily/Hourly loots)");
		sender.sendMessage("§2/" + command
				+ " money [Name] <Amount>§b Set money range to be looted");
		sender.sendMessage("§2/" + command
				+ " exp [Name] <Amount>§b Set experience to be gained");
		sender.sendMessage("§2/"
				+ command
				+ " add cmd [Name] [Percent] /<Command>§b Add a Command that will be executed upon looting");
		sender.sendMessage("§2/"
				+ command
				+ " remove cmd [Name] /<Command>§b Remove a Command that will be executed upon looting");
		sender.sendMessage("§5To add a percent chance to a cmd use % similar to item loot");
		sender.sendMessage("§bex. §1/" + command
				+ " add cmd Test %50 /lightning <player>");
	}
	...
	private static void sendLootHelp(CommandSender sender) {
		...
		sender.sendMessage("§2/" + command
				+ " <add|remove> <Item|ID|hand> [Parameter1] [Parameter2]...");
		sender.sendMessage("§bex. §6/" + command
				+ " add hand #1-16 nEnderNade %32");
		sender.sendMessage("§bex. §6/" + command
				+ " add bow earrow_fire(2) earrow_unlimited %5");
		sender.sendMessage("§bTutorial Video:");
		sender.sendMessage("§1§nwww.youtu.be/tRQuKbRTaA4");
	}

	/**
	 * Returns the a LinkedList of PhatLoots If a name is provided then only the
	 * PhatLoot with the given name will be in the List If no name is provided
	 * then each PhatLoot that is linked to the target Block will be in the List
	 * 
	 * @param sender
	 *            The CommandSender targeting a Block
	 * @param name
	 *            The name of the PhatLoot to be found
	 * @return The a LinkedList of PhatLoots
	 */
	public static LinkedList<PhatLoot> getPhatLoots(CommandSender sender,
			String name) {
		...
		if (name != null) {
			...
			if (phatLoots.isEmpty()) {
				sender.sendMessage("§4Target " + blockName
						+ " is not linked to a PhatLoot");
			}
		}
	 ...
	 * 
	 * @param sender
	 *            The CommandSender that will receive error messages
	 * @param string
	 *            The String that contains the id
	 */
	 ...
	 *            The CommandSender that will receive error messages
	 * @param string
	 *            The String that contains the amount
	 */
	 ...
	 *            The CommandSender that will receive error messages
	 * @param string
	 *            The String that contains the item
	 */
	public static ItemStack getItemStack(CommandSender sender, String string) {
		...
		if (material == null) {
			if (sender != null) {
				sender.sendMessage("§6" + string
						+ "§4 is not a valid item id");
			}
			return null;
		}
		...
		try {
			for (String split : string.split("&")) {
				...
				if (split.contains("(")) {
					...
					level = Integer.parseInt(split.substring(index + 1,
							split.length() - 1));
	 ...
	 *            The CommandSender that will receive error messages
	 * @param string
	 *            The String that contains the percent
	 */
