...
public class PhatLoot {
	public static final int INDIVIDUAL = 0, COLLECTIVE1 = 1, COLLECTIVE2 = 2,
			COLLECTIVE3 = 3, COLLECTIVE4 = 4, COLLECTIVE5 = 5, COLLECTIVE6 = 6,
			COLLECTIVE7 = 7, COLLECTIVE8 = 8, COLLECTIVE9 = 9,
			COLLECTIVE10 = 10;
	...
	public int numberCollectiveLoots = PhatLoots.defaultNumberOfLoots; // Amount
																		// of
																		// loots
																		// received
																		// from
	...
	public LinkedList<String> commands = new LinkedList<String>(); // Commands
																	// that will
																	// be run
																	// upon
																	// looting
	...
	private TreeSet<Loot>[] lootTables = (TreeSet<Loot>[]) new TreeSet[11]; // List
																			// of
																			// items
																			// that
																			// may
	...
	public int days = PhatLoots.defaultDays; // Reset time (will never reset if
												// any are negative)
	...
	private HashSet<PhatLootChest> chests = new HashSet<PhatLootChest>(); // List
																			// of
																			// Chests
																			// linked
																			// to
	 ...
	 * 
	 * @param name
	 *            The name of the PhatLoot which will be created
	 */
	 ...
	 * 
	 * @param player
	 *            The Player who is looting
	 * @param block
	 *            The Block being looted
	 */
	@SuppressWarnings("deprecation")
	public void rollForLoot(Player player, PhatLootChest chest,
			Inventory inventory) {
		...
		if (!timeRemaining.equals("0")) {
			if (displayTimeRemaining) {
				player.sendMessage(PhatLootsMessages.timeRemaining.replace(
						"<time>", timeRemaining));
			}
		...
		if (expUpper > 0) {
			...
			if (amount > 0) {
				...
				player.sendMessage(PhatLootsMessages.experienceLooted.replace(
		...
		boolean itemsInChest = chest.addLoots(lootIndividual(), player,
				inventory);
		...
		if (PhatLoots.autoLoot && !itemsInChest) {
			...
			PhatLoots.closeInventory(player, inventory, chest.getBlock()
					.getLocation(), global);
		}
	...
	public int rollForLoot(Player player, List<ItemStack> drops) {
		...
		if (player != null) {
			// Find out how much time remains
			String timeRemaining = getTimeRemaining(getMobLootTime(player
					.getName()));
			...
			if (!timeRemaining.equals("0")) {
				if (displayMobTimeRemaining) {
					player.sendMessage(PhatLootsMessages.mobTimeRemaining
		...
		if (player != null && !PhatLootsMessages.mobDroppedItem.isEmpty()) {
			for (ItemStack item : loot) {
				player.sendMessage(PhatLootsMessages.mobDroppedItem.replace(
		...
		if (moneyUpper > 0 && player != null) {
			...
			if (amount > 0 && !player.getGameMode().equals(GameMode.CREATIVE)
				...
				String money = Econ.reward(player.getName(), amount);
				player.sendMessage(PhatLootsMessages.mobDroppedMoney.replace(
						"<amount>", money));
			}
		}
	...
	public void rollForLoot(LivingEntity entity) {
		...
		if (loot.size() != 5) {
			PhatLoots.logger.warning("Cannot add loot to "
					+ entity.getType().getName()
					+ " because the amount of loot was not equal to 5");
		}
	...
	/**
	 * Returns the remaining time until the PhatLootChest resets Returns null if
	 * the PhatLootChest never resets
	 * 
	 * @param time
	 *            The given time
	...
	public String getTimeRemaining(long time) {
		...
		time += days * DateUtils.MILLIS_PER_DAY + hours
				* DateUtils.MILLIS_PER_HOUR + minutes
				* DateUtils.MILLIS_PER_MINUTE + seconds
		...
		if (timeRemaining > DateUtils.MILLIS_PER_DAY) {
			...
			return (int) timeRemaining / DateUtils.MILLIS_PER_MINUTE
					+ " minute(s)";
		} else if (timeRemaining > DateUtils.MILLIS_PER_SECOND) {
			return (int) timeRemaining / DateUtils.MILLIS_PER_SECOND
					+ " second(s)";
		} else {
			return "0";
		}
	...
	/**
	 * Executes commands of the PhatLoot Each command is rolled for
	 * 
	 * @param player
	 *            The Player looting
	 */
	public void lootCommands(Player player) {
		...
		for (String cmd : commands) {
			if (cmd.matches(".*%[0-9]*[.]?[0-9]+")) {
				...
				if (PhatLoots.random.nextInt(100)
						+ PhatLoots.random.nextDouble() < percent) {
					PhatLoots.server.dispatchCommand(
							cs,
							cmd.substring(0, index).replace("<player>",
									player.getName()));
				}
			} else {
				PhatLoots.server.dispatchCommand(cs,
	...
	/**
	 * Fills the Chest (Block) with loot Each item is rolled for to determine if
	 * it will by added to the Chest Money is rolled for to determine how much
	 * will be given within the range
	 */
	...
	/**
	 * Fills the Chest (Block) with loot Items are rolled for in order until the
	 * maximum number is added to the Chest
	 */
	public LinkedList<ItemStack> lootCollective() {
		...
		for (int i = 1; i <= 5; i++) {
			// Make sure there are items that will be looted before entering the
			// loop
	 ...
	 * 
	 * @param chest
	 *            The PhatLootChest to set the time for
	 * @param player
	 *            The Player whose time is to be updated
	 */
	public void setTime(PhatLootChest chest, String player) {
		...
		lootTimes.setProperty(chest.toString() + "'" + player,
	 ...
	 * 
	 * @param player
	 *            The Player whose time is requested
	 ...
	 * 
	 * @param id
	 *            The id of the collective Loots
	 * @return Total probability of all Loots in the collective Loots subtracted
	 *         from 100
	 */
	 ...
	 * 
	 * @param id
	 *            The id of the Loots (0 for individual loots)
	 * @param string
	 *            The data of the Loots
	 */
			...
			try {
				...
				if (item.startsWith("(")) {
					...
					if (string.matches("[0-9]+")) {
						color = Color.fromRGB(Integer.parseInt(item.substring(
								1, index)));
					} else {
				...
				if (lootData[1].endsWith("auto")) {
					autoEnchant = true;
					lootData[1] = lootData[1].substring(0,
							lootData[1].length() - 5);
				}
				if (data.contains("+")) {
	...
	public void setChests(String data) {
		...
		for (String chest : data.split(", ")) {
			try {
				...
				PhatLootChest phatLootChest = new PhatLootChest(chestData[0],
						Integer.parseInt(chestData[1]),
						Integer.parseInt(chestData[2]),
						Integer.parseInt(chestData[3]));
	...
	/**
	 * Resets the user times for all PhatLootChests of this PhatLoot If a Block
	 * is given then only reset that PhatLootChest
	 * 
	 * @param block
	 *            The given Block
	 */
	public void reset(Block block) {
		if (block == null) {
			...
			String chest = block.getWorld() + "'" + block.getX() + "'"
	...
	public static String getItemName(ItemStack item) {
		...
		return WordUtils.capitalizeFully(item.getType().toString()
				.replace("_", " "));
	}
