...
/**
 * A PhatLootChest is a Block location and a Map of Users with times attached to
 * them
 * 
 * @author Codisimus
 */
public class PhatLootChest {
	 ...
	 * @param block
	 *            The given Block
	 * @return True if the given Block is the same Dispenser or part of the
	 *         double Chest
	 */
	...
	public boolean addLoots(List<ItemStack> itemList, Player player,
			Inventory inventory) {
	...
	public boolean addLoot(ItemStack item, Player player, Inventory inventory) {
		...
		if (item.getAmount() > item.getMaxStackSize()) {
			...
			addLoot(new ItemStack(id, item.getMaxStackSize(), durability),
					player, inventory);
			addLoot(new ItemStack(id,
					item.getAmount() - item.getMaxStackSize(), durability),
					player, inventory);
		}
		...
		if (isDispenser) {
			...
			player.sendMessage(PhatLootsMessages.autoLoot.replace("<item>",
					PhatLoot.getItemName(item)));
			sack.addItem(item);
			return false;
		} else {
	 ...
	 * @param player
	 *            The Player (if any) that will be informed of the drop
	 */
	public void overFlow(ItemStack item, Player player) {
		Block block = getBlock();
		block.getWorld().dropItemNaturally(block.getLocation(), item);
		if (player != null) {
			player.sendMessage(PhatLootsMessages.overflow.replace("<item>",
					item.getType().name()));
		}
	}

	/**
	 * Returns the String representation of this PhatLootChest The format of the
	 * returned String is world'x'y'z
	 * 
	...
	public boolean equals(Object object) {
		if (object instanceof PhatLootChest) {
			...
			return chest.x == x && chest.y == y && chest.z == z
